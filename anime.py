...
welcome = {
	...
	'inline' : False,
	'buttons' : [
		[get_but('Перелестнуть вперед', 'positive')],
	]
}
...
forma = {
	...
	'buttons' : [
		[get_but('Перелестнуть вперед', 'positive')],
		[get_but('Перелестнуть назад', 'negative')],
	]
}
	...
	if event.type == VkEventType.MESSAGE_NEW:
		...
		if msg == '127':
			if users[user_id].stats == 2:
				...
				vk_session.method('messages.send', {'user_id' : id, 'message' : 'Используйте "Перелестнуть вперед", чтобы перелестнуть', 'keyboard' : welcome, 'random_id' : random.randint(-1234567, +1234567)})
			...
			if users[user_id].stats == 2:
				if users[user_id].number == 127:
					...
					if users[user_id].manga == 1:
						...
						vk_session.method('messages.send', {'user_id' : id, 'message' : 'Используйте "Перелестнуть вперед", чтобы перелестнуть вперед', 'keyboard' : welcome, 'random_id' : random.randint(-1234567, +1234567)})
					if users[user_id].manga == 2:
						photo(id, 'photo-202584478_457239075')
						vk_session.method('messages.send', {'user_id' : id, 'message' : 'Используйте "Перелестнуть вперед", чтобы перелестнуть вперед' + '\nИспользуйте "Перелестнуть назад", чтобы перелестнуть назад', 'keyboard' : forma, 'random_id' : random.randint(-1234567, +1234567)})
